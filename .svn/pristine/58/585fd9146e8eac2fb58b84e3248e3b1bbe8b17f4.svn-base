...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="main_table">
			...
			<c:when test="${not empty Divisions }">
				<c:forEach items="${Divisions}" var="e" varStatus="s">
					<tr class="main_info">
						<td><input type="hidden" value="${e.id}"/>${s.count}</td>				
						<td>${e.divisionName}</td>
						<td><a href="javascript:updateDivision(${e.id})">修改</a></td>
						<td><a href="javascript:deleteDivision(${e.id})">删除</a></td>
					</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
	...
	<script type="text/javascript">
		...
		{
			if(confirm("确定要删除该记录？")){
				var url = "/Home/Division/deleteDivision.do";
		...
		function addDivision(){
			...
			var dg = new $.dialog({
				...
				xButton:true,
				resize:false,
				page:'Division/getDivisionbyId.do?id=0'
				});
    		dg.ShowDialog();
		}
		...
		function updateDivision(id){
			var dg = new $.dialog({
				...
				xButton:true,
				resize:false,
				page:'Division/getDivisionbyId.do?id='+id
				});
    		dg.ShowDialog();
		}
