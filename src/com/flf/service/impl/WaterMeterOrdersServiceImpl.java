...
public class WaterMeterOrdersServiceImpl implements WaterMeterOrdersService{
	...
	@Override
	public void submitWaterMeterOrders(WaterMeterOrders waterMeterOrders) {
		int tmp = waterMeterOrdersMapper.submitWaterMeterOrders(waterMeterOrders.getOrderId());
		if(tmp>0){
			if(waterMeterOrders.getState().equals("3")){
				for (WaterMeterOrderRecords wRecords : waterMeterOrders.getWaterMeterOrderRecordsList()) {
					if(wRecords==null){
						continue;
					}
					MeterReadingData meterReadingData = new MeterReadingData();
					meterReadingData.setBuildingStructureId(wRecords.getBuildingStructureId());
					meterReadingData.setReading(wRecords.getWaterLastReading());
					meterReadingData.setMeterReadingPeople(waterMeterOrders.getPrincipal());
					meterReadingDataMapper.updateReasonByBuildingStructureId(meterReadingData);
				}
			}
		}
	}
