...
public interface StaffMapper {// 员工表
	...
	List<Staff> listStaffByAll(Staff staff);//根据条件模糊查询员工列表（姓名、工号、电话、证件号）
	 ...
	 * 
	 */
	List<Staff> listALlStaffInfo(Staff staff);
	
	/**
	 * 删除员工(state = 0的员工)
	 */
	int deleteStaffStateByStateAndId(Staff staff);
	/**
	 * 删除员工(state = 0的员工)
	 */
	int leaveStaffStateByStateAndId(Staff staff);

	Staff selectByidone(String staffId);
	
	List<Staff> selectByTeamworkStaffAdd(String teamworkId);
}
