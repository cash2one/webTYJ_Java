...
public interface BillMapper {
	...
	/**
	 * 根据项目ID查找所有状态为未核销的账单并合并金额
	 * @author maogaofei
	 * @date 2016年7月28日 下午2:55:24
	 * @param projectId
	 * @return
	 */
	List<Bill> listMergeBillByProjectId(String projectId);
	/**
	 * 历史账单的明细
	 * @param bill
	 * @return
	 */
	List<Bill> findChargeDetailsBychargeTypeIdLastMonth(Bill bill);
	
	List<Bill> listUNMergeBillByProjectId(String projectId);
	/**
	 * 查询所有的账单
	 * @author maogaofei
	 * @date 2016年8月2日 下午6:09:40
	 * @param bill
	 * @return
	 */
	List<Bill> selectAllBill();
	List<Bill> listElectronicBillingByHouseIdAndCustId(String houseId);
	List<Bill> listBillByElectronicBillingId (String buildingId);
	/**
	 * weixin调用查询收费项目的历史
	 * @param bill
	 * @return
	 */
	List<Bill> listBillByElectronicBillingId(Bill bill);
	/**
	 * weixin调用查询账单
	 * @param bill
	 * @return
	 */
	List<Bill> listPageFindChargeBybuildingId(Bill bill);
