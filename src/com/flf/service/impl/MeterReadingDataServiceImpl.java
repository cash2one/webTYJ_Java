...
//						meterReadingProgram.setUnfinishedNum(meterReadingProgram.getUnfinishedNum() - 1);//未抄表数－1
	...
	public PageRestful listPageWaterMeterRecordAll(
		...
		for (MeterReadingData item : meterReadingDatas) {
			...
			if(item.getSpecifiedDate()!=null)
			{
				...
				if(readingData != null){
					if (readingData.getConsumption()!=null) {
		        ...
		        if(readingData2 != null){
		        	if (readingData2.getConsumption()==null) {
		        	...
		        	if (item.getLastYearRead()==null) {
						...
						item.setLastYearRead(readingData2.getLastYearRead());
					}
		        }
		        else{
		        	item.setLastYearUse(0.0);
	...
	public InputStream exportWaterDataReportExcel() {
		// TODO Auto-generated method stub
		        ...
		        for (MeterReadingData item : dataset) {
					//计算上个月的日期
					Calendar calender = Calendar.getInstance();
					if (item.getCorrection()==null) {
						item.setCorrection(0.0);
