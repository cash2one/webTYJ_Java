...
public class SchemeServiceImpl implements SchemeService {
	...
	public String insertScheme(Scheme scheme) {
		...
		sm.setProjectId(scheme.getProjectId());
	...
	public String updateScheme(String schemeId) {
		...
		if (!Tools.isEmpty(schemeId)) {
			...
			for (int i = 0 ; i < array.length; i++) {
				...
				try {
					Date nowDate = sdf.parse(sdf.format(new Date()));
					if(Tools.notEmpty(sm.getEndDate())){
	...
	@Override
	public List<Scheme> getManualSchemeList(Scheme scheme) {
		Date nowDate = new Date();
		SimpleDateFormat sdfMonth = new SimpleDateFormat("yyyy-MM");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String nowDateStr = sdf.format(nowDate);
		List<Scheme> list=schemeMapper.getManualSchemeList(nowDateStr,scheme.getProjectId());
