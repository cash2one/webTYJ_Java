...
public class FinancialBankNumServiceImpl implements FinancialBankNumService{
	...
	public String updateFinancialBankNum(FinancialBankNum financialBankNum) {
		...
		if(!bool){
			...
			if(financialBankNum.getBankNum().length() != 2){
				map.put("code","3001");
				map.put("information","行别长度必须为2位");
			    return JSONObject.fromObject(map).toString();//3001表示参数不全    200表示成功
			}
		}
		if(!bl){
			...
			if(financialBankNum.getUnionpayBankNum().length() != 4){
				map.put("code","3001");
				map.put("information","银联银行行号长度必须为4位");
			    return JSONObject.fromObject(map).toString();//3001表示参数不全    200表示成功
			}
		}
		...
		if(financialBankNum.getBankNameSeq().length() < 3){
			map.put("code","3001");
			map.put("information","简称序列长度必须大于等于3位");
		    return JSONObject.fromObject(map).toString();//3001表示参数不全    200表示成功
		}
